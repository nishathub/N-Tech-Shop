...
const BrandProducts = () => {
    ...
    useEffect(() => {
        const categoryBrands = [...new Set(oneBrandProducts.map(product => product.type))];
        setCategories(categoryBrands);
    }, [oneBrandProducts])

    const handleCategoryButtonClick = (categoryName) => {
        const clickedCategoryProducts = oneBrandProducts.filter(product => product.type === categoryName);
                        ...
                        <div className="h-20 flex gap-4 items-center justify-center px-6 mt-4 mb-12">
                            {categories.map((category) => (
                                <button
                                    ...
                                    className={`font-bold p-2 rounded-md cursor-pointer duration-300 ${selectedCategory === category
